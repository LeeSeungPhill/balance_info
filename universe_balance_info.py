...
if kis_df01.empty:
    ...
    # Streamlit 앱 구성
    st.title("KIS 기간별 잔고현황 조회")

    kis_df01['일자'] = pd.to_datetime(kis_df01['일자']).dt.strftime('%Y-%m-%d')

    # 버튼을 클릭하면, 데이터프레임이 보이도록 만들기.
    if st.button('KIS 기간별 잔고현황 상세 데이터'):

        df_display = kis_df01.sort_values(by='일자', ascending=False).copy().reset_index(drop=True)
    ...
    kis_df01['일자'] = pd.to_datetime(kis_df01['일자'])
    kis_df01 = kis_df01.dropna(subset=['일자'])               
    kis_df01 = kis_df01.sort_values(by='일자')
    kis_df01 = kis_df01[kis_df01['전체금액'] != 0]
    # 인덱스를 'YYYY-MM-DD' 문자열로 포맷
    kis_df01['일자_str'] = kis_df01['일자'].dt.strftime('%Y-%m-%d')
    kis_df01.set_index('일자_str', inplace=True)                
...
if df02.empty:
    ...
    # Streamlit 앱 구성
    st.title("KIS 기간별 누적수익합계")

    df02['일자'] = pd.to_datetime(df02['일자']).dt.strftime('%Y-%m-%d')

    # 버튼을 클릭하면, 데이터프레임이 보이도록 만들기.
    if st.button('KIS 기간별 누적수익합계 상세 데이터'):
...
if not result1:
    ...
    if df1.empty:
        ...
        # Streamlit 앱 구성
        st.title("KIS 기간별 매매 손익현황 조회")

        df1['거래일자'] = pd.to_datetime(df1['거래일자']).dt.strftime('%Y-%m-%d')

        # 버튼을 클릭하면, 데이터프레임이 보이도록 만들기.
        if st.button('KIS 기간별 매매 손익현황 상세 데이터'):
...
if not result2:
    ...
    if df2.empty:
        ...
        # Streamlit 앱 구성
        st.title("KIS 기간별 손익 일별합산 조회")

        df2['거래일자'] = pd.to_datetime(df2['거래일자']).dt.strftime('%Y-%m-%d')

        # 버튼을 클릭하면, 데이터프레임이 보이도록 만들기.
        if st.button('KIS 기간별 손익 일별합산 상세 데이터'):
